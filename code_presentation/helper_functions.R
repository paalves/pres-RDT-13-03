...
#================================================================================#
# Séries temporelles de spectres fréquentiels #
#================================================================================#

plotSpectraTimeSeries <- function(ef, freqValues = NULL, timeValues = NULL,
                                  titleSpectre="Visualisation du spectre") {
  
  # On a ef un tableau tridimensionnel [1, frequency, time].
  spectral_data <- ef[1,,]
  
  library(reshape2)
  plot_data <- melt(spectral_data)
  names(plot_data) <- c("Frequency", "Time", "Power")
  
  obsThisMonth <- dim(ef)[3]
  
  if (is.null(timeValues)) {
    plot_data$Time_Value <- plot_data$Time
  } else {
    plot_data$Time_Value <- timeValues[plot_data$Time]
  }
  
  library(ggplot2)
  
  spectro_plot <- ggplot(plot_data, aes(x = Time_Value, y = Frequency, fill = Power)) +
    geom_tile() +
    scale_fill_gradientn(
      colors = c("navy", "blue", "cyan", "green", "yellow", "orange", "red"),
      name = "Énergie"
    ) +
    theme_minimal() +
    labs(
  ...
  if (!is.null(freqValues)) {
    spectro_plot <- spectro_plot +
      scale_y_continuous(
        breaks = seq(1, length(freqValues), by = 2), # On n'affiche qu'une fréquence sur deux sinon c'est illisible
        labels = freqValues[seq(1, length(freqValues), by = 2)],
        name = "Fréquence (Hz)"
      )
  }
  ...
  time_series <- data.frame(
    Time = seq_len(obsThisMonth),
    TotalPower = colSums(spectral_data, na.rm = TRUE)
  )
  
  ts_plot <- ggplot(time_series, aes(x = Time, y = TotalPower)) +
  ...
  avg_spectrum <- data.frame(
    Frequency = seq_len(dim(spectral_data)[1]),
    AveragePower = rowMeans(spectral_data, na.rm = TRUE)
  )
  
  if (!is.null(freqValues)) {
    avg_spectrum$Frequency <- freqValues
  }
  
  spectrum_plot <- ggplot(avg_spectrum, aes(x = Frequency, y = AveragePower)) +
    geom_line(color = "red") +
    theme_minimal() +
    labs(
      title = "Average Power Spectrum",
      x = if (!is.null(freqValues)) "Frequency" else "Frequency Index",
      y = "Average Power"
    ) +
    theme(
      plot.title = element_text(hjust = 0.5)
  ...
  combined_plot <- grid.arrange(
    spectro_plot, ts_plot, spectrum_plot,
    layout_matrix = rbind(c(1,1,1),
                          c(2,3,3))
  )
  
  return(list(
    specter        = spectro_plot,
    totalPower     = ts_plot,
    avgPower       = spectrum_plot,
    combinedPlot   = combined_plot
  ))
}


#================================================================================#
# Extraction des points près de celui où on a le max d'énergie #
#================================================================================#

extract_near_max_energy <- function(spectre, nObs = 100) {
  
  dims <- dim(spectre)
  
  if (length(dims) == 2) {
    spectre <- array(spectre, dim = c(1, dims[1], dims[2]))
    dims <- dim(spectre)  
  }
  energy_sums <- apply(spectre, c(2, 3), sum) 
  
  max_idx <- which(energy_sums == max(energy_sums, na.rm = TRUE), arr.ind = TRUE)

  target_idx <- max_idx[1, "col"]  
  
  half_window <- floor(nObs / 2)
  start_idx <- max(1, target_idx - half_window)
  end_idx   <- min(dims[3], target_idx + half_window)
  
  selected_spectre <- spectre[ , , start_idx:end_idx, drop = FALSE]

  return(selected_spectre)
}


#================================================================================#
# Extraction de séries temporelles de Hs autour des max de tempêtes #
#================================================================================#

analyser_tempetes <- function(data, seuil_indices = 72) {
  ...
  #---------------------------#
  # Fonction interne: identifier_tempetes
  #---------------------------#
  
  identifier_tempetes <- function(data, seuil_indices = 72) {
    pics <- which(!is.na(data$hs) & data$hs > 6)
    
    if(length(pics) == 0) {
      return(NULL) 
    }
    
    cat("Nombre de points avec hs > 6:", length(pics), "\n")
    ...
    tempete_courante <- c(pics[1])
    
    for(i in 1:length(diff_pics)) {
      if(diff_pics[i] >= seuil_indices) {
        
        # Nouvelle tempête
        tempetes[[length(tempetes) + 1]] <- tempete_courante
        tempete_courante <- c(pics[i+1])
      } else {
        
        # Continuation de la tempête courante
        tempete_courante <- c(tempete_courante, pics[i+1])
      }
    }
    
    # Ajouter la dernière tempête
    tempetes[[length(tempetes) + 1]] <- tempete_courante
  ...
  #---------------------------#
  # Fonction interne: creer_groupes_tempetes
  #---------------------------#
  
  creer_groupes_tempetes <- function(data, tempetes) {
    if(is.null(tempetes) || length(tempetes) == 0) {
      return(NULL)
    }
    
    groupes <- list()
    
    for(i in 1:length(tempetes)) {
      indices_tempete <- tempetes[[i]]
      indices_tempete <- indices_tempete[indices_tempete <= nrow(data)]
      
      if(length(indices_tempete) == 0) next
      
      # Trouver l'indice avec la valeur hs maximale
      hs_values <- data$hs[indices_tempete]
      if(all(is.na(hs_values))) next
      
      max_index_in_subset <- which.max(hs_values)
      max_hs_index <- indices_tempete[max_index_in_subset]
      
      # Déterminer les indices des 50 observations avant et après (modifiable)
      debut <- max(1, max_hs_index - 50)
      fin <- min(nrow(data), max_hs_index + 50)
      
      # Créer un groupe avec les observations
      groupe <- data[debut:fin, ]
      groupe$tempete <- paste("Tempête", i)
      groupe$temps_relatif <- seq(-50, fin - debut - 50)  # Tracé centré sur le pic
      
      groupes[[i]] <- groupe
    }
    
    # Combiner tous les groupes en un seul dataframe
    groupes_non_null <- Filter(function(x) !is.null(x), groupes)
    if(length(groupes_non_null) > 0) {
      resultat <- do.call(rbind, groupes_non_null)
      return(resultat)
    }
  ...
  cat("Nombre de valeurs NA dans hs:", sum(is.na(data$hs)), "\n")
  
  tempetes <- identifier_tempetes(data, seuil_indices = seuil_indices)
  
  if(is.null(tempetes)) {
    cat("Aucune tempête identifiée.\n")
  } else {
    cat("Nombre de tempêtes identifiées:", length(tempetes), "\n")
    
    for(i in 1:length(tempetes)) {
      cat("Tempête", i, "- Nombre de pics:", length(tempetes[[i]]))
      
      indices_valides <- tempetes[[i]][tempetes[[i]] <= nrow(data)]
      if(length(indices_valides) > 0) {
        hs_values <- data$hs[indices_valides]
        if(!all(is.na(hs_values))) {
          cat(" - Max hs:", max(hs_values, na.rm = TRUE))
          max_index <- indices_valides[which.max(hs_values)]
          cat(" - Date pic max:", format(data$time[max_index], "%Y-%m-%d %H:%M:%S"))
        }
      }
      cat("\n")
    }
  ...
  groupes_tempetes <- tryCatch({
    creer_groupes_tempetes(data, tempetes)
  }, error = function(e) {
    cat("Erreur lors de la création des groupes:", e$message, "\n")
    return(NULL)
  })
  
  if(is.null(groupes_tempetes)) {
    cat("Aucun groupe de tempête n'a pu être créé.\n")
    return(invisible(NULL))
  }
  
  groupes_tempetes$tempete <- as.factor(groupes_tempetes$tempete)
  levels(groupes_tempetes$tempete) <- c("Tempête d'octobre 1996", 
                                        "Tempête de janvier 1998",
                                        "Tempête Johanna (2008)",
                                        "Tempête Eleanor (2018)",
                                        "Tempête Ciaran (2023)")
  
  tryCatch({
    p1 <- ggplot(groupes_tempetes, aes(x = time, y = hs, color = tempete, group = tempete)) +
      geom_line(size = 1) +
      geom_point() +
      theme_minimal() +
      labs(title = "Évolution de hs pour chaque tempête identifiée",
           x = "Date et heure",
           y = "Hauteur significative (hs)",
           color = "Tempête") +
      theme(legend.position = "bottom") +
      geom_hline(yintercept = 6, linetype = "dashed", color = "red", alpha = 0.7) +
      facet_wrap(~ tempete, scales = "free_x", ncol = 2)
    
    print(p1)
    
    p2 <- ggplot(groupes_tempetes, aes(x = time, y = hs, color = tempete)) +
      geom_line(size = 1) +
      geom_point() +
      theme_minimal() +
      labs(title = "Évolution de hs pour toutes les tempêtes identifiées",
           x = "Date et heure",
           y = "Hauteur significative (hs)",
           color = "Tempête") +
      theme(legend.position = "bottom") +
    ...
    p3 <- ggplot(groupes_tempetes, aes(x = temps_relatif, y = hs, color = tempete, group = tempete)) +
      geom_line(size = 1) +
      theme_minimal() +
      labs(title = "Évolution de Hs au cours des tempêtes où Hs dépasse 6m",
           x = "Nombre d'observations par rapport au pic (0 = pic)",
           y = "Hauteur significative (hs)",
           color = "Tempête") +
      facet_wrap(~ tempete) +
      theme(legend.position = "bottom") +
      geom_hline(yintercept = 6, linetype = "dashed", color = "red", alpha = 0.7) +
      geom_vline(xintercept = 0, linetype = "dotted", color = "blue", alpha = 0.7) +
      theme_bw()
  ...
  return(invisible(groupes_tempetes))
}
